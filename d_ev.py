...
_MACHEPS = cp.finfo(np.float64).eps
...
class DifferentialEvolutionSolver(object):
    ...
    def __init__(self, func, bounds, args=(),
                 ...
                 maxfun=cp.inf, callback=None, disp=False, polish=True,
                 init='latinhypercube', atol=0):
        cp.cuda.Stream.null.synchronize()
        ...
        self.scale = mutation
        print(type(mutation))
        mutation=cp.asarray(mutation)
        print(mutation)
        if (not cp.all(cp.isfinite(mutation)) or
                cp.any(cp.array(mutation) >= 2) or
                cp.any(cp.array(mutation) < 0)):
            cp.cuda.Stream.null.synchronize()
        ...
        self.limits = cp.array(bounds, dtype='float').T
        cp.cuda.Stream.null.synchronize()
        if (cp.size(self.limits, 0) != 2 or not
                cp.all(cp.isfinite(self.limits))):
            cp.cuda.Stream.null.synchronize()
        ...
        self.__scale_arg2 = cp.abs(self.limits[0] - self.limits[1],dtype=float)
        cp.cuda.Stream.null.synchronize()
        self.parameter_count = cp.size(self.limits, 1)
        cp.cuda.Stream.null.synchronize()
    ...
    def init_population_lhs(self):
        ...
        # Offset each segment to cover the entire parameter range [0, 1)
                   + cp.linspace(0., 1., self.num_population_members,
                                 endpoint=False)[:, cp.newaxis])
        cp.cuda.Stream.null.synchronize()

        # Create an array for population of candidate solutions.
        self.population = cp.zeros_like(samples)
        cp.cuda.Stream.null.synchronize()
        ...
        self.population_energies = (cp.ones(self.num_population_members) *
                                    cp.inf)
        cp.cuda.Stream.null.synchronize()
    ...
    def init_population_array(self, init):
        ...
        popn = cp.asarray(init,dtype=float)
        cp.cuda.Stream.null.synchronize()
        if (cp.size(popn, 0) < 5 or
        ...
        self.population = cp.clip(self._unscale_parameters(popn), 0, 1)
        cp.cuda.Stream.null.synchronize()
        self.num_population_members = cp.size(self.population, 0)
        cp.cuda.Stream.null.synchronize()
        self.population_shape = (self.num_population_members,
                                 self.parameter_count)
    ...
    def convergence(self):
        ...
        mean.
        """
        return (cp.std(self.population_energies) /
                cp.abs(cp.mean(self.population_energies) + _MACHEPS))
    ...
    def solve(self):
        ...
        # cp.inf). If it has you have to calculate the initial energies.
        # Although this is also done in the evolve generator it's possible
        # that someone can set maxiter=0, at which point we still want the
        # initial energies to be calculated (the following loop isn't run).
        if cp.all(cp.isinf(self.population_energies)):
        ...
        for nit in xrange(1, self.maxiter + 1):
            ...
            intol = (cp.std(self.population_energies) <=
                     self.atol +
                     self.tol * cp.abs(cp.mean(self.population_energies)))
    ...
    def _calculate_population_energies(self):
        ...
        parameters = cp.array([self._scale_parameters(c) for c in candidates]) # TODO: vectorize
        cp.cuda.Stream.null.synchronize()
        ...
        minval = cp.argmin(self.population_energies)
        cp.cuda.Stream.null.synchronize()
    ...
    def __next__(self):
        ...
        trials = cp.array([self._mutate(c) for c in range(itersize)]) # TODO: vectorize
        cp.cuda.Stream.null.synchronize()
        for trial in trials: self._ensure_constraint(trial)
        parameters = cp.array([self._scale_parameters(trial) for trial in trials])
        cp.cuda.Stream.null.synchronize()
        energies = self.func(parameters, *self.args)
        self._nfev += itersize
    ...
    def _ensure_constraint(self, trial):
        ...
        """
        for index in cp.where((trial < 0) | (trial > 1))[0]:
            cp.cuda.Stream.null.synchronize()
    ...
    def _mutate(self, candidate):
        ...
        """
        trial = cp.copy(self.population[candidate])
        cp.cuda.Stream.null.synchronize()
        rng = self.random_number_generator
        ...
        if self.strategy in self._binomial:
            ...
            trial = cp.where(crossovers, bprime, trial)
            cp.cuda.Stream.null.synchronize()
            return trial
    ...
    def _randtobest1(self, samples):
        ...
        """
        r0, r1, r2 = samples[:3]
        bprime = cp.copy(self.population[r0])
        cp.cuda.Stream.null.synchronize()
