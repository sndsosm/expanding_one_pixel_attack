import os
import sys
import numpy as np

import argparse

import torch
import torch.nn as nn
import torch.optim as optim
import torch.nn.functional as F
import torch.backends.cudnn as cudnn
from cifar10_models.resnet import resnet18,resnet34,resnet50
from cifar10_models.vgg import vgg13_bn,vgg11_bn,vgg16_bn,vgg19_bn
from cifar10_models.densenet import densenet121,densenet161,densenet169
from cifar10_models.mobilenetv2 import mobilenet_v2
from cifar10_models.googlenet import googlenet
from cifar10_models.inception import inception_v3
import torchvision
import torchvision.transforms as transforms
from torch.autograd import Variable
import torch as T
import torchvision as tv
import torchvision.transforms as transforms
import matplotlib.pyplot as plt
import numpy as np
from random import randint, randrange
from scipy.optimize import dual_annealing, basinhopping
def imshow(img):
  img = img / 2 + 0.5   # unnormalize
  npimg = img.numpy()   # convert from tensor
  plt.imshow(np.transpose(npimg, (1, 2, 0))) 
  plt.show()
from differential_evolution import differential_evolution
device = 'cuda' if torch.cuda.is_available() else 'cpu'
model_defs = {
        'resnet18': resnet18,
        'resnet34': resnet34,
        'resnet50': resnet50,
        'vgg11': vgg11_bn(pretrained=True).to(device),
        'vgg13': vgg13_bn,
        'vgg16': vgg16_bn,
        'vgg19': vgg19_bn,
        'densenet121':densenet121,
        'densenet161':densenet161,
        'densenet169':densenet169,
        'mobilenet':mobilenet_v2,
        'googlenet':googlenet,
        'inception':inception_v3,
    }
parser = argparse.ArgumentParser(description='One pixel attack with PyTorch')
parser.add_argument('--model',default='resnet18', choices=model_defs.keys(), type=str)
parser.add_argument('--pixels', default=1, type=int, help='The number of pixels that can be perturbed.')
parser.add_argument('--method',  type=int,default=1,
                        help='Specify optimization algorithm.')
parser.add_argument('--maxiter', default=100, type=int, help='The maximum number of iteration in the DE algorithm.')
parser.add_argument('--popsize', default=400, type=int, help='The number of adverisal examples in each iteration.')
parser.add_argument('--temperature', default=5230, type=int,
                        help='The initial temperature for dual annealing algorithm. Increasing this number requires more computation.')
parser.add_argument('--T', default=1.0, type=int,
                        help='The temperature for basin hopping algoirthm. Increasing this number requires more computation.')
parser.add_argument('--samples', default=100, type=int, help='The number of image samples to attack.')
parser.add_argument('--targeted', action='store_true', help='Set this switch to test for targeted attacks.')
parser.add_argument('--save', default='./results/results.pkl', help='Save location for the results with pickle.')
parser.add_argument('--save_attack', default='networks/results/attack_results.pkl', help='Save location for the results (pickle)')
parser.add_argument('--verbose', action='store_true', help='Print out additional information every iteration.')

args = parser.parse_args()

def perturb_image(xs, img):
  if xs.ndim < 2:
    xs = np.array([xs])
  batch = len(xs)
  imgs = img.repeat(batch, 1, 1, 1)
  xs = xs.astype(int)
  count = 0
  for x in xs:
    pixels = np.split(x, len(x)//9)
    for pixel in pixels:
    
      x_pos1, y_pos1, red, x_pos2, y_pos2, green, x_pos3, y_pos3, blue= pixel
      imgs[count, 0, x_pos1, y_pos1] = (red/255.0-0.4914)/0.2023
      imgs[count, 1, x_pos2, y_pos2] = (green/255.0-0.4822)/0.1994
      imgs[count, 2, x_pos3, y_pos3] = (blue/255.0-0.4465)/0.2010
    count += 1
  return imgs


def predict_classes(xs, img, target_calss, net, minimize=True):
  imgs_perturbed = perturb_image(xs, img.clone())
  input = Variable(imgs_perturbed, volatile=True).cuda()
  predictions = F.softmax(net(input)).data.cpu().numpy()[:, target_calss]

  return predictions if minimize else 1 - predictions

def attack_success(x, img, target_calss, net, targeted_attack=False, verbose=False):

  attack_image = perturb_image(x, img.clone())
  input = Variable(attack_image, volatile=True).cuda()
  confidence = F.softmax(net(input)).data.cpu().numpy()[0]
  predicted_class = np.argmax(confidence)

  if (verbose):
    print ("Confidence: %.4f"%confidence[target_calss])
  if (targeted_attack and predicted_class == target_calss) or (not targeted_attack and predicted_class != target_calss):
    return True

def attack(img, label, net, target=None,method=1, pixels=1, maxiter=75, popsize=400,temperature=5230.0, T=1, verbose=False):
  # img: 1*3*W*H tensor
  # label: a number
  targeted_attack = target is not None
  target_calss = target if targeted_attack else label
  predict_fn = lambda xs: predict_classes(
    xs, img, target_calss, net, target is None)
  callback_fn = lambda x, convergence: attack_success(
    x, img, target_calss, net, targeted_attack, verbose)
  callback_fn_DA=lambda x,f, context: attack_success(
    x, img, target_calss, net, targeted_attack, verbose)
  callback_fn_BH=lambda x,f, accept: attack_success(
    x, img, target_calss, net, targeted_attack, verbose)
  if (method==1):
          alg='DE'
          bounds =[(0,31), (0,31),(0,255), (0,31), (0,31),(0,255),(0,31), (0,31),(0,255)]* pixels
          popmul = max(1, popsize//len(bounds))
          inits = np.zeros([popmul*len(bounds), len(bounds)])
          print(inits.shape)
          for init in inits:
            for i in range(pixels):
             # print(init[i*5+0])
              init[i*9+0] = np.random.random()*31
             # print(init[i*5+0])
              init[i*9+1] = np.random.random()*31
        
              init[i*9+2] = np.random.normal(128,127)

              init[i*9+3] = np.random.random()*31

              init[i*9+4] = np.random.random()*31
              
              init[i*9+5] = np.random.normal(128,127)

              init[i*9+6] = np.random.random()*31

              init[i*9+7] = np.random.random()*31

              init[i*9+8] = np.random.normal(128,127)
          attack_result = differential_evolution(predict_fn, bounds, maxiter=maxiter, popsize=popmul,
            recombination=1, atol=-1, callback=callback_fn, polish=False, init=inits)
  elif (method==2):
          alg='DA'
          bounds = [(0,31), (0,31),(0,255), (0,31), (0,31),(0,255),(0,31), (0,31),(0,255)]* pixels
          
          attack_result =dual_annealing(predict_fn, bounds, maxiter=maxiter, intital_temp=temperature, callback=callback_fn_DA)
  elif (method==3):
          alg='BH'
          bounds =[(0,31), (0,31),(0,255), (0,31), (0,31),(0,255),(0,31), (0,31),(0,255)]* pixels
          popmul = max(1, popsize//len(bounds))
          minimizer_kwargs = { "method": "L-BFGS-B","bounds":bounds }
          inits=[randrange(0,31),randrange(0,31),randint(0,255),randrange(0,31),randrange(0,31),randrange(0,255),randrange(0,31),randrange(0,31),randrange(0,255)]*pixels
          attack_result = basinhopping(predict_fn,inits,niter=maxiter,T=T, callback=callback_fn_BH)
 
  attack_image = perturb_image(attack_result.x, img)
  imshow(tv.utils.make_grid(attack_image))
  attack_var = Variable(attack_image, volatile=True).cuda()
  predicted_probs = F.softmax(net(attack_var)).data.cpu().numpy()[0]
  predicted_class = np.argmax(predicted_probs)
  if (not targeted_attack and predicted_class != label) or (targeted_attack and predicted_class == target_calss):
    return 1, attack_result.x.astype(int)
  return 0, [None]

def attack_all(net, loader, pixels=1,method=1, targeted=False, maxiter=75, popsize=400,temperature=5230.0, T=1, verbose=False):

  correct = 0
  success = 0

  for batch_idx, (input, target) in enumerate(loader):

    img_var = Variable(input, volatile=True).cuda()
    prior_probs = F.softmax(net(img_var))
    _, indices = torch.max(prior_probs, 1)
    
    if target[0] != indices.data.cpu()[0]:
      continue

    correct += 1
    target = target.numpy()

    targets = [None] if not targeted else range(10)

    for target_calss in targets:
      if (targeted):
        if (target_calss == target[0]):
          continue
      
      flag, x = attack(input, target[0], net, target_calss, method,pixels=pixels, maxiter=maxiter, popsize=popsize, verbose=verbose)

      success += flag
      if (targeted):
        success_rate = float(success)/(9*correct)
      else:
        success_rate = float(success)/correct

      if flag == 1:
        print ("success rate: %.4f (%d/%d) [(x,y) = (%d,%d) and (R,G,B)=(%d,%d,%d)]"%(
          success_rate, success, correct, x[0],x[1],x[2],x[3],x[4]))
    
    if correct == args.samples:
      break

  return success_rate

def main():
  print ("==> Loading data and model...")
  tranfrom_test = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
    ])
  test_set = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=tranfrom_test)
  testloader = torch.utils.data.DataLoader(test_set, batch_size=1, shuffle=True, num_workers=2)
  class_names = ['plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck']
  cudnn.benchmark = True
  print ("==> Starting attck...")
  base = [model_defs[m](pretrained=True) for m in [args.model]]
  results = attack_all(net=base[0].to(device), loader=testloader, pixels=args.pixels, method=args.method, targeted=args.targeted, maxiter=args.maxiter, popsize=args.popsize,temperature=args.temperature, T=args.T, verbose=args.verbose)
  print ("Final success rate: %.4f"%results)

if __name__ == '__main__':
  main()